...
const quantityRoomsFree = computed((type='ida') => {
  return roomsFree.value[type].reduce((acc, room) => {
...
const updateWidth = () => {
  ...
  matrizRooms.value = allRooms(props.data);
};
...
function postReserva(room){
  const params = new URLSearchParams();
  params.append('trecho_id', step === 1 ? props.dataIda.id : props.dataVolta.id);
  params.append('viagem_id', step === 1 ? props.dataIda.id : props.dataVolta.id);
...
function deleteReserva(room){
  routes['rooms.reservas.delete']({
    data:{
      trecho_id: step === 1 ? props.dataIda.id : props.dataVolta.id,
      viagem_id: step === 1 ? props.dataIda.id : props.dataVolta.id,
      comodo_ids:[room.id]
    }
  }).then((response) => {
    if(response.data.success){
          ...
          <div class="tw-flex tw-flex-col my-3 tw-w-full" v-if="openRooms">
            ...
            <v-row >
              ...
              <v-col cols="12" md="6" class="tw-hidden lg:tw-block">
                ...
                <div v-if="roomsSelected.selectedsByType?.length >  0 || roomsSelected.selectedsById?.length >  0" class="tw-flex tw-gap-3 mt-5">
                  ...
                  <v-btn variant="outlined" color="success" rounded  class="d-lg-flex  !tw-font-extrabold px-2 py-1" size="xs" @click="initSale">
                    <span class=" !tw-text-xs "  >Escolher volta</span><Icon icon="mdi:navigate-next" width="20"  class="ml-1 "  />
                  </v-btn>
                </div>
              </v-col>
            </v-row>
